...
<div class="crf-main-form">
  ...
  <form name="crf_form" id="crf_form" method="post">
   ...
   <div class="crf-form-setting" >
      ...
      <div class="crf-form-right-area crf_required">
        <input type="text" name="form_name" id="form_name" value="<?php if(!empty($row)) echo esc_attr($row->form_name); ?>" tabindex="3" onKeyUp="check('<?php if(!empty($row)){echo esc_attr($row->form_name);}else { echo 'new';} ?>')"/>
    ...
    <div class="crf-form-setting">
      ...
      <div class="crf-form-right-area">
        <textarea name="form_des" id="form_des" tabindex="4"><?php if(!empty($row))echo esc_attr($row->form_desc); ?></textarea>
      </div>
    </div>
  ...
  <div class="crf-form-setting">
    ...
    <div class="crf-form-setting crf_lock_feature" id="letuserdecidehtml" style=" <?php if(!empty($row) && $row->form_type=='reg_form'){echo 'display:block;';} else { echo 'display:none;';} ?>">
      ...
      <div class="crf-form-right-area">
        <input name="let_user_decide" id="let_user_decide" type="checkbox" tabindex="1" class="upb_toggle" value="" style="display:none;" disabled />
    ...
    <div class="crf-form-setting" id="userrolelabelhtml" style=" <?php if(!empty($let_user_decide) && $let_user_decide=='1'){echo 'display:block;';} else { echo 'display:none;';} ?>">
      ...
      <div class="crf-form-right-area">
        <input name="user_role_label" id="user_role_label" type="text" tabindex="1" value="<?php if (!empty($user_role_label)){ echo esc_attr($user_role_label);}?>" />
        <div class="custom_error"></div>
      </div>
    </div>
    ...
    <div class="crf-form-setting" id="userroleoptionhtml" style=" <?php if(!empty($let_user_decide) && $let_user_decide=='1'){echo 'display:block;';} else { echo 'display:none;';} ?>">
	  ...
	  $roles = get_editable_roles();
          ...
          <input type="checkbox" name="user_role_options[]" id="user_role_options[]" value="<?php echo $key;?>" tabindex="5" <?php if(!empty($user_role_options) && in_array($key,$user_role_options)) echo 'checked';?>>
    ...
    <div class="crf-form-setting crf_lock_feature">
      ...
      <div class="crf-form-right-area">
        <input name="show_token" id="show_token" type="checkbox" tabindex="10" class="upb_toggle" value="1" style="display:none;" disabled/>
    ...
    <div class="crf-form-setting">
      ...
      <div class="crf-form-right-area crf_radiorequired">
        <label>
          <input type="radio" name="redirect_option" id="redirect_option_page" value="none" onClick="showhideredirect(this.value)" <?php if(!empty($row) && $row->redirect_option=='none') echo 'checked'; ?> tabindex="5">
...
$args = array(
    ...
    <div class="crf-form-setting autoresponder"  style="display:<?php if (!empty($row) && $row->send_email==1){ echo "block";}else {echo 'none';}?>">
      ...
      <div class="crf-form-right-area">
        <input type="text" name="welcome_email_subject" id="welcome_email_subject" tabindex="11" value="<?php if(!empty($row)) echo esc_attr($row->crf_welcome_email_subject); ?>" />
      </div>
    </div>
    <div class="crf-form-setting autoresponder" style="display:<?php if (!empty($row) && $row->send_email==1){ echo "block";}else {echo 'none';}?>">
      <div class="crf-form-left-area">
    ...
    <div class="crf-form-setting">
      ...
      <div class="crf-form-right-area crf_submit_label">
       <input type="text" name="submit_button_label" id="submit_button_label" placeholder="Submit" tabindex="14" value="<?php if(isset($submit_button_label)) echo esc_attr($submit_button_label);?>" />
       <div class="custom_error"></div>
      </div>
    </div>
    ...
    <div class="crf-form-setting optin_box_html" style="display:<?php if(isset($optin_box) && $optin_box==1){ echo "block";}else {echo 'none';}?>">
      ...
      <div class="crf-form-right-area">
        <textarea name="optin_box_text" id="optin_box_text" tabindex="16"><?php if(!empty($optin_box_text)) echo esc_attr($optin_box_text); ?></textarea>
    ...
    <div class="crf-form-setting" id="limitation_submission_html" style="display:<?php if(isset($expiry_type) && ($expiry_type=='submission' || $expiry_type=='both')) echo 'block'; else echo 'none';?>;">
      ...
      <div class="crf-form-right-area crf_number">
         <input type="text" name="submission_limit" id="submission_limit" tabindex="19" value="<?php if(isset($submission_limit)) echo esc_attr($submission_limit); ?>" />
         <div class="custom_error"></div>
      </div>
    </div>
    ...
    <div class="crf-form-setting" id="limitation_date_html" style="display:<?php if(isset($expiry_type) && ($expiry_type=='date' || $expiry_type=='both')) echo 'block'; else echo 'none';?>;">
      ...
      <div class="crf-form-right-area crf_datepicker">
         <input type="text" name="expiry_date" id="expiry_date" tabindex="20" value="<?php if(isset($expiry_date)) echo esc_attr($expiry_date); ?>" class="crf_date rm_tcal" />
         <div class="custom_error"></div>
      </div>
    </div>
    ...
    <div class="crf-form-setting" id="expiry_message_html">
      ...
      <div class="crf-form-right-area">
         <textarea name="expiry_message" id="expiry_message" tabindex="21"><?php if(isset($expiry_message)) echo esc_attr($expiry_message); ?></textarea>
         <div class="custom_error"></div>
      </div>
    </div>
