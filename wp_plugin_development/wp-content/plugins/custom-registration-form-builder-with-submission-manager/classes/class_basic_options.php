...
<?php	
		...
		public function crf_get_sumission_token_number($formid,$id)
		{
			...
			if(isset($form_option['showtoken']))
			{
...
<table class="crf_user_submissions">
  <tr>
    ...
    <th class="crf_form_title">Form Title</th>
    <th class="crf_submission_date">Date</th>
    <th class="crf_form_payment">Payment</th>
    <th class="crf_view_submission"></th>
  </tr>
  <?php $i=1; foreach($submissions as $submission):?>
  <tr>
    <td class="crf_user_sr"><?php echo $i;?></td>
    <td class="crf_form_title"><?php echo $this->crf_get_form_option_value('form_name',$submission->form_id);?></td>
    ...
    <td class="crf_view_submission"><a href="admin.php?page=crf_view_entry&id=<?php echo $submission->submission_id;?>">View</a></td>
  </tr>
  <?php $i++; endforeach;?>
</table>
<?php	
...
<li class="header rows">
  <div class="cols" style="width:30px;"></div>
  <div class="cols" style="width:30px;">#</div>
  <div class="cols" style="width:50px;"></div>
  <div class="cols" style="width:19%;">Username</div>
  <div class="cols" style="width:30%;">Email</div>
  <div class="cols" style="width:19%;">Name</div>
  <div class="cols" style="width:50px;"></div>
</li>
<?php
...
<li class="<?php echo $class;?> rows">
  ...
  <div class="cols" style="width:30px;"><a href="admin.php?page=crf_view_wp_user&id=<?php echo $entry->ID;?>"><?php echo $i; ?></a></div>
  <div class="cols crf_userimage" style="width:50px"><a href="admin.php?page=crf_view_wp_user&id=<?php echo $entry->ID;?>"><?php echo $avatar;?></a></div>
  <div class="cols" style="width:19%"><?php echo $entry->user_login;?></div>
  <div class="cols" style="width:30%"><?php echo $entry->user_email;?></div>
  <div class="cols" style="width:19%"><?php echo $entry->display_name;?></div>
  <div class="cols" style="width:50px"><a href="admin.php?page=crf_view_wp_user&id=<?php echo $entry->ID;?>">View</a></div>
</li>
<?php
			  $i++;
			}
			...
			foreach($reg as $row)
			{
				...
				if(trim($value)!="" || !empty($value))
				{
					if($row->Type=='file')
					{
							  ...
							  $html .=  '<div class="formtable">
							<label>'.$label.'</label>';	
							  foreach($values as $fileid)
							  {  
								
								$html .= '<span class="user_details user_details_img">'.wp_get_attachment_link($fileid,'full',false,true,false).'</span>';
					...
					else
					{
							$html .= '<div class="formtable">
							<label>'.$label.'</label>
							<span class="user_details">'.$value.'</span>
							</div>';	
					}
				}
